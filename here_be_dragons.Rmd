...
## Load dependencies
```{r}
#remotes::install_github("traitecoevo/hmde@bf1acfc0ec2e2c43d421d017fbeb28c44ad63940", force = TRUE)
...
### Optimisation tests
To better understand the posterior distribution shape we use the BFGS optimiser in Stan to test a grid of points around the posterior for a couple of different cases. 
```{r}
#Load model
affine_model <- stan_model(file = "stan/affine_single_ind.stan")

#-----------------------------------------------------------------------------#
#RK4 with no grid, 0.5 step size
...
#-----------------------------------------------------------------------------#
#Analytic solution with no grid
rstan_data <- hmde_model("affine_single_ind") |>
      hmde_assign_data(n_obs = nrow(obs_data_frame),
                       y_obs = obs_data_frame$y_obs,
                       obs_index = obs_data_frame$obs_index,
...
#Grid
beta_0_grid <- seq(from = 9, to = 11, length.out = 100)
beta_1_grid <- seq(from = 0.8, to = 1.2, length.out = 100)

save_data <- opt_init_grid(rstan_data, beta_0_grid, beta_1_grid)

saveRDS(save_data, file = "output/analytic_grid_optimizing.rds")

#-----------------------------------------------------------------------------#
#RK4 with two grids, 0.5 step size
rstan_data <- hmde_model("affine_single_ind") |>
      hmde_assign_data(n_obs = nrow(obs_data_frame),
                       y_obs = obs_data_frame$y_obs,
                       obs_index = obs_data_frame$obs_index,
...
#Good parameter cluster
beta_0_grid <- seq(from = 8, to = 12, length.out = 100)
beta_1_grid <- seq(from = 0.8, to = 1.2, length.out = 100)

save_data <- opt_init_grid(rstan_data, beta_0_grid, beta_1_grid)

saveRDS(save_data, file = "output/rk4_cluster1_optimizing.rds")

#Bad parameter cluster
beta_0_grid <- seq(from = 49, to = 51, length.out = 100)
beta_1_grid <- seq(from = 4.9, to = 5.1, length.out = 100)

save_data <- opt_init_grid(rstan_data, beta_0_grid, beta_1_grid)

saveRDS(save_data, file = "output/rk4_cluster2_optimizing.rds")
...
#Grid
beta_0_grid <- seq(from = 8, to = 12, length.out = 100)
beta_1_grid <- seq(from = 0.7, to = 1.3, length.out = 100)

save_data <- opt_init_grid(rstan_data, beta_0_grid, beta_1_grid)

saveRDS(save_data, file = "output/rk45_optimizing.rds")
