...
beta_0_grid <- seq(from = 8, to = 12, length.out = 50)
beta_1_grid <- seq(from = 0.8, to = 1.2, length.out = 50)
inits <- expand.grid(beta_0 = beta_0_grid, 
                     beta_1 = beta_1_grid)
rk4_cluster1_post_est <- tibble()
rk4_cluster1_hessian <- list()

for(i in 1:nrow(inits)){
  fit <- optimizing(affine_model, rstan_data, alg = "BFGS",
                    hessian = TRUE, 
                    init = list(beta_0 = inits$beta_0[i], 
                                beta_1 = inits$beta_1[i]))
  
  est_tibble_temp <- tibble(
    run = i,
    beta_0_init = inits$beta_0[i],
    beta_1_init = inits$beta_1[i],
    beta_0_est = fit$par[[beta_0]],
    beta_1_est = fit$par[[beta_1]]
  )
  
  rk4_cluster1_post_est <- rbind(rk4_cluster1_post_est, est_tibble_temp)
  rk4_cluster1_hessian[[i]] <- fit$hessian
}
