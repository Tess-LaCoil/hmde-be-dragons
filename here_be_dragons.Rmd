...
good_plot <- traceplot(good_fit, pars = c("ind_beta_0", "ind_beta_1"), 
                       colour = "green4") +
  labs(title = "Good estimate traceplots")
bad_plot <- traceplot(bad_fit, , pars = c("ind_beta_0", "ind_beta_1"),
                      colour = "cornflowerblue") +
  labs(title = "Bad estimate traceplots")
...
```{r}
affine_numeric_poly <- function(x = NULL, pars = NULL){ #Pars: h, y_0, y_1
  y <- pars[[2]] - pars[[3]] + 
           (10 - pars[[2]])*( pars[[1]]*x - (pars[[1]]^2/2)*x^2 + 
...
for(i in 2:5){
  pars <- list(beta_0 = 10, beta_1 = 1, y_0 = pars_tibble$y_1[i-1])
  pars_tibble_temp <- tibble(
    step = i,
    h = 0.5,
    y_0 = pars_tibble$y_1[i-1],
    y_1 = analytic_solution(0.5, pars)
  )
...
for(i in 1:5){
  args_list = list(pars = as.numeric(pars_tibble[i,2:4]))
...
beta_0_lims <- c(min(c(
      rk45_par_est_tibble$beta_0,
       analytic_par_est_tibble$beta_0
     )), max(c(
       rk45_par_est_tibble$beta_0,
       analytic_par_est_tibble$beta_0
)))
beta_1_lims <- c(min(c(
      rk45_par_est_tibble$beta_1,
       analytic_par_est_tibble$beta_1
     )), max(c(
       rk45_par_est_tibble$beta_1,
       analytic_par_est_tibble$beta_1
)))
...
rk45_hist_1_xlim <- rk45_hist_1 + 
  xlim(beta_1_lims)
analytic_hist_0_xlim <- analytic_hist_0 +
  xlim(beta_0_lims)
analytic_hist_1_xlim <- analytic_hist_1 + 
  xlim(beta_1_lims)

plot_grid(
  rk45_hist_0_xlim,
  rk45_hist_1_xlim,
  analytic_hist_0_xlim,
  analytic_hist_1_xlim,
  align = "vh",
  ncol = 2,
  labels = lab_vec_4
)

#Old data with log-normal prior
plot_grid(
  rk45_hist_old_0,
  rk45_hist_old_1,
  nrow = 1,
  align = "vh",
  labels = lab_vec_4[1:2]
)
