...
#Get some extreme estimates
cluster_1 <- rk45_par_est_tibble %>%
  filter(beta_0 <= mean(rk45_par_est_tibble$beta_0))
cluster_2 <- rk45_par_est_tibble %>%
  filter(beta_0 > mean(rk45_par_est_tibble$beta_0))
...
for(i in 1:length(beta_prior_testing)){
  ...
  #Get some initialising estimates
  cluster_1 <- prior_test_est_tibble %>%
    filter(beta_0 <= mean(prior_test_est_tibble$beta_0))
  cluster_2 <- prior_test_est_tibble %>%
    filter(beta_0 > mean(prior_test_est_tibble$beta_0))
...
dens_prior_est_table_rounded <- dens_prior_est_table %>%
  mutate(
    ...
    cluster_dist = signif(cluster_dist, digits = 4),
