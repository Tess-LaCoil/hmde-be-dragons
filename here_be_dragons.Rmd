...
saveRDS(indep_err_numeric_par_est_tibble, file = "output/analytic_par_est_tibble.rds")
```
...
for(i in 1:length(unique(par_est_tibble$step_size))){
  ...
  step_size_mix_models_par_ests_temp <- tibble(
    ...
    m_dist_c1 = mahalanobis(step_size_mix_models[[i]]$mu[[1]], 
                            center = m_dist_centre, 
                            cov = step_size_mix_models[[i]]$sigma[[1]]),
    m_dist_c2 = mahalanobis(step_size_mix_models[[i]]$mu[[2]], 
                            center = m_dist_centre, 
                            cov = step_size_mix_models[[i]]$sigma[[2]]),
    euclid_dist = dist(dist_table),
...
write.csv(step_size_mix_models_par_ests, "output/step_size_mix_models_par_ests.csv")
```
...
print("Cluster 1 M. dist")
mahalanobis(rk45_mix_models$mu[[1]],
            center = m_dist_centre,
            cov = rk45_mix_models$sigma[[1]])
print("Cluster 2 M. dist")
mahalanobis(rk45_mix_models$mu[[2]],
            center = m_dist_centre,
            cov = rk45_mix_models$sigma[[2]])
...
#Fit multivariate normal finite mixture model to the estimates
mix_model_analytic <- fit_mix_model(x = analytic_par_est_tibble[,c(3,4)])
...
for(i in 1:length(beta_prior_testing)){
  ...
  prior_test_dist_table_temp <- tibble(
    prior = c(),
    m_dist_c1 = mahalanobis(mix_model_prior_list[[i]]$mu[[1]], 
                            center = m_dist_centre, 
                            cov = mix_model_prior_list[[i]]$sigma[[1]]),
    m_dist_c2 = mahalanobis(mix_model_prior_list[[i]]$mu[[2]], 
                            center = m_dist_centre, 
                            cov = mix_model_prior_list[[i]]$sigma[[2]]),
...
#Fit multivariate normal finite mixture model to the estimates
analytic_indep_mix_models <- fit_mix_model(indep_err_par_est_tibble[,c(3,4)])
...
for(i in 1:length(optimizing_tests)){ #Plots
  ...
  #sqrt_det_vec <- c()
  #for(j in 1:nrow(est_tibble)){
  #  sqrt_det_vec[j] <- sqrt(det(data[[2]][[j]]))
  #}
